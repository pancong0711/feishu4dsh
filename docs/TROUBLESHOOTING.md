...
## 4. dsh web 打开/驱动飞书通道会话——输出错乱，甚至飞书端无法继续（单写入方约定）

**结论先行**：一个宿主会话只应有单一写入方。**请不要在 dsh web 中打开 `feishu-*` 前缀的会话并发送消息**——那会让 web 与飞书通道同时驱动同一会话。

### 4.1 为什么危险

- 桥层按「会话归属」而非「回合发起方」渲染事件：web 发起的回合会被当成聊天自己的输出流进群聊卡片，聚合摘要/Token 口径随之错位；
- 通道假定每会话单句柄：两端各跑一轮时流互相抢占、审批路由串扰；
- 最重后果：交错写同一会话日志触发上游封存竞态（end-seed / seq 撞号）→ **会话日志损坏，飞书端此后每条消息都失败**，需换新会话或处置损坏会话目录才能恢复。
- 敞口背景：v0.6.0 起通道会话会记账到工作区分组（修复 web「未分组」），`feishu-*` 会话因此在 web 列表可见可点——可见 ≠ 可安全双驱动。

### 4.2 症状速查

| 表现 | 判断 |
|---|---|
| 话题里出现"没头没尾"的回复/摘要数字异常 | 疑似 web 侧同会话有回合在跑 |
| `/new` 之前每条消息都报错、且日志见会话写入冲突痕迹 | 会话日志可能已损坏 |
| 群聊/私聊突然持续无响应，仅该话题受影响 | 先排除本条（换 `/new` 验证） |
